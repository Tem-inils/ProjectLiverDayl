...
def start_message(message):
    ...
    if check_user:
        ...
        bot.send_message(user_id, f'Добро пожаловать! {message.from_user.first_name}', reply_markup=bt.remove())
...
def get_loc(message, user_name, user_num):
    ...
    if message.location:
        ...
        bot.send_message(user_id, f'{user_id} успешно зарегистрировались!')
        products = db.get_pr_name_id()
        bot.send_message(user_id, 'Выберите пункт меню', reply_markup=bt.main_menu_buttons(products))
